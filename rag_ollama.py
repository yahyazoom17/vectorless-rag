...
def llm_tree_search(query: str, tree: list, model: str = "gemma4:latest") -> dict:
    """
...
Reply ONLY in this JSON format with no extra text, no markdown, no code fences:
{{
  ...
  "node_list": ["node_id1", "node_id2"]
    ...
    response = openai_client.chat.completions.create(
        ...
        #response_format={"type": "json_schema", "json_schema": {"type": "object", "properties": {"thinking": {"type": "string"}, "node_list": {"type": "array", "items": {"type": "string"}}}, "required": ["thinking", "node_list"]} },
    )
...
def generate_answer(query: str, nodes: list, model: str = "gemma4:latest") -> str:
    if not nodes:
...
_ = vectorless_rag(
    query="How to perform 'ESTIMATION OF ACID MIXTURE BY CONDUCTOMETRIC SENSOR' experiment?",
    tree=pageindex_tree,
)